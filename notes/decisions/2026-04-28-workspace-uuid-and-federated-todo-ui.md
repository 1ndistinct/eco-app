# Workspace And Todo UUIDs And Federated Todo UI
...
- Todo identifiers are UUIDs at the service and frontend boundary.
- Fresh installs create UUID workspace references directly in `workspaces`, `workspace_memberships`, and `todos`.
- Upgrades preserve existing bigint workspace and todo keys only as compatibility columns (`legacy_id`, `legacy_workspace_id`) during migration.
...
- UUID todo IDs remove the remaining database-local autoincrement dependency from the todo service.
...
## Consequences
- Backend stores validate workspace IDs and todo IDs as UUIDs.
...
- Frontend routing and API payloads continue to treat workspace IDs and todo IDs as opaque strings.
