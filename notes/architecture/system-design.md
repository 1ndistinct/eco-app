...
The backend now follows the same boundary: `services/app-shell` owns auth/workspaces/shares with its own internal runtime packages, while `services/app-todo` owns `/api/todos` plus the todo migration set with its own internal runtime packages.
